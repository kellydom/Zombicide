...
public class CameraController : MonoBehaviour {
	...
	void Update () {
		...
		if((Input.GetKey(KeyCode.Q) ||Input.GetKeyDown(KeyCode.PageUp) || Input.GetAxis("Mouse ScrollWheel") > 0) && transform.position.y > closeDist)
		{
			transform.Translate(Vector3.forward * zoomSpeed);
		}
		
		// Mouse wheel moving backward
		if((Input.GetKey(KeyCode.E) || Input.GetKeyDown(KeyCode.PageDown) || Input.GetAxis("Mouse ScrollWheel") < 0) && transform.position.y < farDist)
		{
